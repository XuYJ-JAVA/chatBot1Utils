...
HYPERPARAMETERS = {"MAX_SEQ_LEN": 32, "EMBEDDING_SIZE": 128, "ENCODER_HIDDEN_SIZE": 128,
                   ...
                   "LR_RATE": 1e-2, "KEEP_PROB": 0.5, "EPOCHS": 20, "BATCH_SIZE": 20, "STEP_TO_SAVE": 20
                   }
...
class Seq2SeqModel(object):
    ...
    def dump_dictionary(self, pkl_file_path):
        ...
        dump到文件 保存字典
        """
        dictionary_info = {"id_to_char": self.id_to_char, "char_to_id": self.char_to_id, "vocab_size": self.vocab_size}
            ...
            for i in range(epochs):
                ...
                for batch_data in self.data_provider.gen_next_batch(batch_size):
                    ...
                    if is_debug and step % step_to_save == 0:
                        for j in range(min(10, len(sample_ids))):
...
def main(data):
    ...
    model_dir = os.path.join("chatBot1Utils", "model")
    model_name = "generate_model.ckpt"
    dictionary_file = os.path.join("chatBot1Utils", "model", "dictionary.pkl")

    if stage == "train":
        chat_file = os.path.join("chatBot1Utils", "data", "chat_qa.txt")
        ...
        for answer in seq2seq_model.inference(model_dir, [question]):
            return answer
